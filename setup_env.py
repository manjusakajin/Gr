...
class SetupENV():
  def __init__(self, master=None, screen = None):
    ...
    self.demo_turtle_screen = TurtleScreen(demo_screen)
  ...
  def chance_bgcolor(self, frame=None):
    ...
    self.width_entry.place(x=50, y=180)
    self.height_entry.place(x=50, y=210)
    ...
    var_name = Entry(self.master, width = 20)
    var_name.place(x=100,y=330)
    Label(frame, text='Default:').place(x=20,y=360)
    default = Entry(self.master, width = 20)
  ...
  def _getColor(self):
    color = askcolor()
    self.demo_turtle_screen.bgcolor(color[1])
    self.bgpic = None
  ...
  def _getPicture(self):
    ...
    if fl != '':
      ...
      img.save(fl)
      self.bgpic = fl
      self.demo_turtle_screen.bgpic(self.bgpic)
  ...
  def _setup(self):
    ...
    if self._is_int(width) and self._is_int(height):
      ...
      self.demo_turtle_screen.mode(mode="world")
      self.demo_turtle_screen.setworldcoordinates(0,0, self.width, self.height)
      self.pixel_width = 670/self.width
      self.pixel_height = 520/self.height
      self._show_grid()
  def _is_int(self, x):
    try:
        x = int(x)
  ...
  def _show_grid(self):
    canvas = self.demo_turtle_screen.getcanvas()
    for x in range(self.width):
      canvas.create_line(x*self.pixel_width,-520,x*self.pixel_width,0)
    for y in range(self.height):
      canvas.create_line(0,-y*self.pixel_height,670,-y*self.pixel_height)
  ...
  def _convert_coodinate(self, turtlex, turtley):
    ...
    canvas_coor["lowx"] = turtlex*self.pixel_width
    canvas_coor["lowy"] = - turtley*self.pixel_height
    canvas_coor["upx"] = (turtlex+1)*self.pixel_width
    canvas_coor["upy"] =  - (turtley+1)*self.pixel_height
    return canvas_coor
  def _add_variable(self, name, default):
    if name != "":
      self.variable[name] = default
    print(self.variable)
    variablelist = Listbox(self.master, width =15, height=5)
